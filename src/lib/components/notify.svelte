<script lang="ts">
  import "./notify.css";
  import type { MessageType } from "$lib/store";
  import { notify } from "$lib/store";

  type MessageTypeOnly = Pick<MessageType, "type">;

  export let message = "",
    id: number,
    type: MessageTypeOnly,
    dismissable: boolean;

  $: class_list = `notify shadow-lg ${type}`;

  const remove_notification = (id: number) => {
    document.getElementById(id.toString())?.classList.add("message-removed");
    setTimeout(() => {
      notify.remove(id);
    }, 170); // 170ms is the animation speed
  };
</script>

<div class={class_list} id={id.toString()}>
  {message}
  <button
    on:click={() => {
      remove_notification(id);
    }}
    class="ml-auto"
  >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="1.7em"
      height="1.7em"
      viewBox="0 0 36 36"
    >
      <path
        d="M18 2a16 16 0 1 0 16 16A16 16 0 0 0 18 2m8 22.1a1.4 1.4 0 0 1-2 2l-6-6l-6 6.02a1.4 1.4 0 1 1-2-2l6-6.04l-6.17-6.22a1.4 1.4 0 1 1 2-2L18 16.1l6.17-6.17a1.4 1.4 0 1 1 2 2L20 18.08Z"
        class="clr-i-solid clr-i-solid-path-1"
      />
      <path fill="none" d="M0 0h36v36H0z" />
    </svg>
  </button>
</div>
